...
public class RetornoController {
	...
	public ResponseEntity<Retorno> criarRetorno (@RequestBody @Valid Retorno newRetorno) {
		...
		if (paciente.isPresent() && usuario.isPresent()) {
			...
			newRetorno.setUsuarioNome(usuario.get().getNome());
	...
	@PutMapping("/{id}")
	public ResponseEntity<Retorno> alterarRetorno (@PathVariable Integer id, @RequestBody @Valid Retorno retornoAtualizado) {
		...
		if (retorno.isPresent() && paciente.isPresent() && usuario.isPresent()) {
			...
			retornoAtualizado.setUsuarioNome(usuario.get().getNome());
